...
namespace DynamicFlooring
{
    public partial class ModEntry : Mod
    {
                ...
                if (list.Any())
                {
                    for (int i = list.Count - 1; i >= 0; i--)
                    {
                        if (list[i].area.Contains(point))
                        {
                            ...
                            if (Game1.player.currentLocation == Utility.getHomeOfFarmer(Game1.MasterPlayer))
                            {
                                ReloadMissingAssetsFH(Utility.getHomeOfFarmer(Game1.MasterPlayer));
                            }
                            foreach (Cabin cabin in GetLocations().OfType<Cabin>())
                                if (Game1.player.currentLocation == cabin)
                                {
                                    ReloadMissingAssetsCB(cabin);
                                }
                            Helper.Input.Suppress(e.Button);
                            break;
                        }
        ...
        private static void ReloadMissingAssetsFH(FarmHouse farmHouse)
        {

            if (farmHouse.upgradeLevel >= 1 && farmHouse.HasNpcSpouseOrRoommate())
            {
        ...
        [EventPriority((EventPriority)int.MinValue)]
        private static void ReloadMissingAssetsCB(Cabin cabin)
        {
            if (cabin.upgradeLevel >= 1 && cabin.HasNpcSpouseOrRoommate())
            {
                cabin.showSpouseRoom();
            }
            if (cabin.upgradeLevel >= 3)
            {
                cabin.AddCellarTiles();
