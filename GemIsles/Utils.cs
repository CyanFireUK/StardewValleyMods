...
namespace GemIsles
{
    internal class Utils
    {
        ...
        internal static void CreateIslesMap(GameLocation location)
        {
            ...
            location.waterTiles = new WaterTiles(location.map.Layers[0].LayerWidth, location.map.Layers[0].LayerHeight);
            ...
            foreach (Rectangle isleBox in isleBoxes)
            {
                ...
                if (Game1.random.NextDouble() < Config.TreasureChance)
                {
                    ...
                    location.overlayObjects[randFreeSpots[taken]] = new Chest(new List<Item>() { MineShaft.getTreasureRoomItem() }, randFreeSpots[taken], false, 0, false);
                    taken++;
                }
                ...
                if (Game1.random.NextDouble() < Config.FaunaChance)
                {
                    ...
                    for (int i = 0; i < fauna; i++)
                    {
                        ...
                        location.dropObject(new Object(index.ToString(), 1, false, -1, 0), position * 64f, Game1.viewport, true, null);
                    }
                    taken += fauna;
                }
                if (Game1.random.NextDouble() < Config.ArtifactChance)
                {
                    ...
                    {
                        Vector2 position = randFreeSpots[i + taken];
                        location.objects.Add(position, new Object("590", 1, false, -1, 0));
                    }
                    taken += artifacts;
                }
                ...
                if (Game1.random.NextDouble() < Config.MineralChance)
                {
                    ...
                    for (int i = 0; i < minerals; i++)
                    {
                        ...
                        {
                            location.terrainFeatures.Add(position, new Tree(1 + Game1.random.Next(2).ToString(), 1));
                        }
