...
namespace RobinWorkHours
{
    ...
    public partial class ModEntry : Mod
    {
        ...
        private void GameLoop_DayStarted(object sender, StardewModdingAPI.Events.DayStartedEventArgs e)
        {
            ...
            if (!Config.EnableMod || Utility.isFestivalDay(Game1.dayOfMonth, Game1.season))
                return;
        ...
        {
            if (!Config.EnableMod || !Game1.IsMasterGame || Utility.isFestivalDay(Game1.dayOfMonth, Game1.season) || (!Game1.getFarm().isThereABuildingUnderConstruction() && Game1.player.daysUntilHouseUpgrade.Value <= 0 && (Game1.getLocationFromName("Town") as Town).daysUntilCommunityUpgrade.Value <= 0))
                return;
            var robin = Game1.getCharacterFromName("Robin");
            if(robin is null)
            {
            ...
            {
                startedWalking = true;
                if (robin.currentLocation.Name == dest && (int)robin.Tile.X == destX && (int)robin.Tile.Y == destY)
                {
            ...
            else if(e.NewTime >= Config.EndTime && robin.currentLocation == Game1.getFarm())
            {
                ...
                for(int i = schedulesStrings.Length - 1; i >= 0; i--)
                {
                    ...
                    if (Game1.timeOfDay > travelTime)
                    {
                        ...
                        schedule.Add(Game1.timeOfDay, (SchedulePathDescription)AccessTools.Method(typeof(NPC), "pathfindToNextScheduleLocation").Invoke(robin, new object[] { robin.ScheduleKey, "BusStop", 10, 23, parts[1], x, y, facing, animation, message }));
                        startIndex = i + 1;
                        break;
                    }
                }
                if(startIndex < schedulesStrings.Length)
                {
                    ...
                    for (int i = startIndex; i < schedulesStrings.Length; i++)
                    {
                        ...
                        else
                        {
                            ...
                            schedule.Add(time, (SchedulePathDescription)AccessTools.Method(typeof(NPC), "pathfindToNextScheduleLocation").Invoke(robin, new object[] { robin.ScheduleKey, lastLoc, lastX, lastY, parts[1], x, y, facing, animation, message }));
                        }
                        lastLoc = parts[1];
                        lastX = x;
                        lastY = y;
